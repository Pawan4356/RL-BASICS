...
def calculateReward(state):
    ...
    if -0.5 <= x <= 0.5 and -1 <= xDot <= 1 and -0.07 <= theta <= 0.07 and -0.525 <= thetaDot <= 0.525:
        reward = 1.0
    return reward
...
for episode in range(NUM_EPISODES):
    ...
    while not done:
        ...
        nextObs, _, terminated, truncated, _ = env.step(action)
        done = bool(terminated or truncated)
        reward = calculateReward(nextObs)
    ...
    if (episode + 1) % 5 == 0 or episode == 0:
        pd.DataFrame(metrics).to_csv("cartpoleMetric.csv", index=False)
    print(f"Episode {episode+1:3d}/{NUM_EPISODES} | Reward: {totalReward:6.2f} | Eps: {EPSILON:5.3f}")

env.close()
torch.save(qNet.state_dict(), "DQNCartpole.pt")
print("Training complete — Model saved as 'DQNCartpole.pt'")
