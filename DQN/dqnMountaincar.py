...
for episode in range(NUM_EPISODES):
    ...
    if (episode + 1) % 5 == 0 or episode == 0:
        pd.DataFrame(metrics).to_csv("mountaincarMetric.csv", index=False)
    print(f"Episode {episode+1:4d}/{NUM_EPISODES} | Reward: {totalReward:6.2f} | Eps: {EPSILON:5.3f}")

env.close()
torch.save(qNet.state_dict(), "DQNMountaincar.pt")
print("Training complete — Model saved as 'DQNMountaincar.pt'")
